...
首页访问地址：[http://xyzweb.cyou/](http://xyzweb.cyou/)
...
项目里的 `prompts/` 目录保存了从基础页面、风格调整到功能增强的提示词示例。写提示词时可以按下面方式表达：

- 描述「你想要什么」，而不是「怎么实现」。
- 先给出最核心的需求，后续再迭代细节。
- 如果第一次生成不满意，可以直接说「重新生成一版，这次要更 XXX」。
- 大胆用感性描述，比如「像 XX 那种感觉」「更高级一点」「太素了加点颜色」。
- 对颜色、布局、动画可以直接描述想要的氛围。
- 不满意就继续说「再调调，XX 部分不太对」，像跟设计师沟通一样。
- 功能需求要具体，不要只说「加个按钮」，要说清楚按钮做什么、放在哪里。
- 交互细节很重要，说明点击后的反馈，例如提示、动画或跳转。
